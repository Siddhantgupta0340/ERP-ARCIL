...
import { CheckCircle2, Eye, PauseCircle, Search, X, XCircle } from 'lucide-react';
...
function ApprovalDetail({ item, onClose }: { item: WorkflowItem; onClose: () => void }) {
	...
	const feeDetails = [
		...
		['Amount variance', money(score.amountVariance)],
		['Quantity variance', score.qtyVariance],
	];
				...
				<div className="flex items-start justify-between gap-4 border-b border-white/10 p-4 sm:p-5">
					<div>
						<div className="flex flex-wrap items-center gap-2">
							...
							<Badge tone={matchTone(score.quality)}>{score.score}% Match</Badge>
						</div>
						...
						<section className="space-y-4">
							...
							<div className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
								<div className="flex items-center justify-between gap-3">
									<div className="text-sm font-semibold text-white">3-way matching details</div>
									<Badge tone={matchTone(score.quality)}>{score.label}</Badge>
								</div>
								<div className="mt-4"><Progress value={score.score} tone={matchTone(score.quality)} /></div>
								<div className="mt-4 grid gap-3 sm:grid-cols-3">
									<DetailField label="Amount score" value={`${score.amountScore}%`} />
									<DetailField label="Quantity score" value={`${score.quantityScore}%`} />
									<DetailField label="Document score" value={`${score.documentScore}%`} />
								</div>
							</div>

							<div className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
								<div className="text-sm font-semibold text-white">Related breakdown data</div>
	...
	/>
							...
							<div className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
								...
								<div className="mt-4 grid gap-3">
									...
									<DetailField label="Hold required" value={item.matchStatus === 'Variance' || score.quality === 'variance' ? 'Yes' : 'No'} />
								</div>
							</div>
						</aside>
					</div>
...
export default function ApprovalsPage() {
	...
	return (
		<div className="space-y-5">
			<Panel title="Phase 4: Approval workflow" subtitle="Amount-based routing with live approve, reject, hold, and detail-view actions.">
			...
			<Panel title={`Approval table (${myRows.length})`} subtitle="The eye action opens related invoice, fee, vendor, matching, and approval metadata.">
						...
						<tbody>{myRows.map((item) => {
							...
							return (
								<tr key={item.id} className="transition hover:bg-white/[0.03]">
									...
									<td className="border-b border-white/5 px-3 py-4"><div className="min-w-[160px]"><div className="mb-2 flex items-center justify-between gap-2"><Badge tone={matchTone(score.quality)}>{score.score}%</Badge><span className="text-xs text-slate-500">{item.matchStatus}</span></div><Progress value={score.score} tone={matchTone(score.quality)} /></div></td>
